...
public class Solarise : AdjustableFilter
{
    required public init(modifier: Double) {
        super.init(modifier: modifier)
        self.minimumTreshold = 0
        self.maximumTreshold = 255
        self.modifier = modifier
    }
    ...
    override func formula(var pixel: Pixel) -> Pixel {
        if pixel.red < UInt8(modifier) {
            pixel.red = 255 - pixel.red
        }
        if pixel.green < UInt8(modifier) {
            pixel.green = 255 - pixel.green
        }
        if pixel.blue < UInt8(modifier) {
...
public class ReverseSolarise : AdjustableFilter
{
    var reverseModifier: Double {
        return 255 - modifier
    }
    ...
    override func formula(var pixel: Pixel) -> Pixel {
        if pixel.red > UInt8(reverseModifier) {
            pixel.red = 255 - pixel.red
        }
        if pixel.green > UInt8(reverseModifier) {
            pixel.green = 255 - pixel.green
        }
        if pixel.blue > UInt8(reverseModifier) {
            pixel.blue = 255 - pixel.blue
        }
        return pixel
    }
...
public class Red : AdjustableFilter
{
    required public init(modifier: Double) {
        super.init(modifier: modifier)
        self.minimumTreshold = 0
...
public class Green : AdjustableFilter
{
    required public init(modifier: Double) {
        super.init(modifier: modifier)
        self.minimumTreshold = 0
...
public class Blue : AdjustableFilter
{
    required public init(modifier: Double) {
        super.init(modifier: modifier)
        self.minimumTreshold = 0
